...
const HotSearchScreen: React.FC = () => {
    ...
    const [zaobaoNewsChina, setZaobaoNewsChina] = useState<NewsItem[]>([]);
    ...
    const fetchZaoBaoNews = async () => {
        try {
            ...
            const html = await fetchData('https://www.zaobao.com/realtime');
            const rawItemsStr = await NativeModules.RNHelper.parseNews('zaobao', 'list', html, '(//div[contains(@class, \'lg:w-1/2\')])[2]//a[./span]');
    ...
    const fetchZaoBaoNewsChina = async () => {
        try {
            setLoading(prev => ({ ...prev, zaobaoChina: true }));
            setErrors(prev => ({ ...prev, zaobaoChina: '' }));

            const html = await fetchData('https://www.zaobao.com/realtime');
            const rawItemsStr = await NativeModules.RNHelper.parseNews('zaobao', 'list', html, '(//div[contains(@class, \'lg:w-1/2\')])[1]//a[./span]');
            const rawItems = JSON.parse(rawItemsStr);
            ...
            for (let i = 0; i < Math.min(rawItems.length, displayNum); i++) {
                ...
                items.push({
                    ...
                    link: item.href.startsWith('/')
                        ? 'https://www.zaobao.com' + item.href
                        : 'https://www.zaobao.com/' + item.href,
                    pubDate: item.date,
                });
            }
            ...
            setZaobaoNewsChina(items);
            setLoading(prev => ({ ...prev, zaobaoChina: false }));
        } catch (error) {
            setErrors(prev => ({
                ...prev,
                zaobaoChina: `加载新闻数据失败: ${error.message}`,
            }));
            setLoading(prev => ({ ...prev, zaobaoChina: false }));
        }
    };
    ...
    const refreshAllData = () => {
        ...
        setting.weibo.hotSearchs.includes('zaobao-china') && tasks.push(fetchZaoBaoNewsChina());
    ...
    const openNewsDetail = async (source:string, item:NewsItem) => {
        let uid = 0;
        if(source === 'zaobao' || source === 'zaobao-china'){
            uid = 101;
                    ...
                    {loading ? (
                        ...
                        data.map((item, index) => (
                            <TouchableOpacity
                                ...
                                onPress={() => ['zaobao', 'zaobao-china', 'nytimes', 'rfi'].includes(source) ? openNewsDetail(source, item) : openLink(item.link)}
                            >
                                ...
                                <View style={styles.listItemMeta}>
                                    {(source === 'zaobao' || source === 'zaobao-china' || source === 'wsj' || source === 'nytimes' || source === 'rfi') &&
            ...
            <View style={styles.grid}>
                {setting.weibo.hotSearchs.includes('zaobao') && (
                    <View style={styles.gridItem}>
                        {renderNewsList('联合早报', zaobaoNews, loading.zaobao, errors.zaobao, 'zaobao')}
                    </View>
                )}

                {setting.weibo.hotSearchs.includes('zaobao-china') && (
                    <View style={styles.gridItem}>
                        {renderNewsList('联合早报-国内  ', zaobaoNewsChina, loading.zaobaoChina, errors.zaobaoChina, 'zaobao-china')}
                    </View>
                )}
