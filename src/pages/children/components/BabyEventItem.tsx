...
import {childrenList, Event, EventType, newBornEvents} from '@/src/service/children/model';
...
export default function BabyEventItem({ event, isLast, onDeleted }:{event:Event, isLast:boolean, onDeleted?:()=>void}) {
    ...
    switch (event.eventType) {
        case EventType.Eat:
            extras.push(`${(event as any).amount} ml`);
            break;
        case EventType.Poop:
            extras.push(`${(event as any).type}/${(event as any).color}`);
            break;
        case EventType.Pee:
            extras.push(`${(event as any).level}`);
            break;
        case EventType.Cry:
        ...
        <TouchableOpacity activeOpacity={0.9} onLongPress={handleDelete} delayLongPress={500} style={[styles.logItem, !isLast && styles.logItemBorder]}>
            ...
            <View style={styles.logContent}>
                <View style={styles.logHeader}>
                    <Text style={styles.logChild}>{childrenList[event.child as keyof typeof childrenList]} </Text>
