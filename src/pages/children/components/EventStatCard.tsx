...
import {EventType} from '@/src/service/children/model';
...
// ========== 状态标签 ==========
function StatusTag({ status, onPress }: { status: string, onPress: () => void }) {
    const c = STATUS_TAG_CONFIG[status as keyof typeof STATUS_TAG_CONFIG] || STATUS_TAG_CONFIG.normal;
    return (
                                ...
                                <View key={index} style={styles.thresholdBlock}>
                                    ...
                                    <View style={{ gap: 6}}>
                                        {['normal','warning','alert'].map(level => {
                                            const range = item.level[level as keyof Threshold];
                                            const isCurrent = item.number >= range[0] && item.number <= range[1];
                                            return (
                                                <View key={level} style={[styles.row, isCurrent && {borderRadius: 4, backgroundColor: THRESHOLD_COLORS[level as keyof typeof THRESHOLD_COLORS].bg}]}>
                                                    <Text style={{color: THRESHOLD_COLORS[level as keyof typeof THRESHOLD_COLORS].text, fontSize: 12}}>{level === 'normal' ? '正常' : level === 'warning' ? '预警' : '告警'}</Text>
                                                    <Text style={[styles.thresholdText, isCurrent && {color: THRESHOLD_COLORS[level as keyof typeof THRESHOLD_COLORS].text, fontWeight: '700'}]}>{isCurrent ? ' • 当前' : ''} {range[0]} ~ {range[1]} {item.unit}</Text>
                                                </View>
                                            );
                                        })}
                                    </View>
...
export default function EventStatCard({eventType, eventAlias, events = [], days}: 
    {eventType: string, eventAlias: string, events: any[], days: number}) {
    const [tooltipVisible, setTooltipVisible] = useState(false);

    const config = EventTypes[eventType as keyof typeof EventTypes] || { icon: '📝', color: '#666', bg: '#F5F5F5' };

    // 在组件内部 count
    const count = events.length;

    const getDynamicThresholds = (THRESHOLD: any):Threshold => {
        return {
            normal: [
    ...
    const getStatus = (threshold: Threshold, number: number):string => {
    ...
    const countThreshold = getDynamicThresholds(DAILY_THRESHOLD_CONFIG[eventType as keyof typeof DAILY_THRESHOLD_CONFIG].count);
    thresholds.push({
        title: '当前次数 ' + count + ' 次',
        number: count,
        unit: '次',
    ...
    switch (eventType) {
        case EventType.Eat:
            let amount = events.reduce((sum, event) => sum + ((event as any).amount || 0), 0);
            displayNumberText = `${amount} ml`;

            const amountThreshold = getDynamicThresholds(DAILY_THRESHOLD_CONFIG[eventType as keyof typeof DAILY_THRESHOLD_CONFIG].amount);
            thresholds.push({
        ...
        case EventType.Sleep:
            let duration = events.reduce((sum, event) => sum + (event.duration || 0), 0);
            displayNumberText = `${duration} 分`;

            let durationThreshold = getDynamicThresholds(DAILY_THRESHOLD_CONFIG[eventType as keyof typeof DAILY_THRESHOLD_CONFIG].duration);
            thresholds.push({
