                            ...
                            <View style={styles.modalContent}>
                                ...
                                {Object.entries(options).map(([key, value]) => (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            styles.modalOption,
                                            selectedValue === key && styles.modalOptionSelected
                                        ]}
                                        onPress={() => {
                                            onValueChange(key);
...
// ========== 日期输入组件 ==========
function DateInput({ label, value, onChange }: {label: string, value: Date, onChange: (date: Date) => void}) {
                ...
                {Object.keys(eventsByType).length === 0 ? (
                    ...
                    <View style={styles.statsGrid}>
                        {Object.entries(newBornEvents).map(([eventType, eventAlias]) => {
                            ...
                            return (
                                <EventStatCard key={eventType} eventType={eventType} eventAlias={eventAlias as string} events={eventList} days={days}/>
                            );
                        })}
                    </View>
                )}
            ...
            <View style={styles.detailHeaderRow}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipContainer}>
                    {Object.entries(newBornEvents).map(([key, label]) => {
                        ...
                        return (
                            <TouchableOpacity key={key} style={[styles.chip, active && {backgroundColor: EventTypes[key as EventType]?.bg || '#eee'}]}
                                onPress={() => {
                                    if (active) {
                            ...
                            >
                                <Text style={[styles.chipText, active && {color: EventTypes[key as EventType]?.color || '#333'}]}>
                                    {label as string}
                                </Text>
                            </TouchableOpacity>
                        );
