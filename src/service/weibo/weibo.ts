...
export default class WeiboService{
    ...
    public async createWeibo(uid:string, content:string, media: Media[], location:Location|null, repostWeibo: Weibo|null):Promise<[boolean, string]>{
        try {
            ...
            await weiboDB.createWeibo(weibo);
            return [true, ''];
        }catch (e){
    ...
    public async getWeiboByPage(uid:string, page:number, offset:number, limit:number, keyword:string = '', startDate?:string, endDate?:string, sortOrder:string = 'desc'):Promise<[boolean, Weibo[], string]>{
        ...
        try {
            let rows:any = [];
            if (keyword === '') {
                rows = await weiboDB.getWeiboByPage(uids, page, offset, limit, ['1', '2'], startDate, endDate, sortOrder);
            } else {
                rows = await weiboDB.getWeiboByPageWithKeyword(uids, page, offset, limit, ['1', '2'], keyword, startDate, endDate, sortOrder);
            }
    ...
    public async getComments(id:string):Promise<[boolean, Comment[], string]>{
        ...
        try {
            const rows = await weiboDB.getComments(id);
            const commentIds = [];
            const replyIds = [];
            const userIds = [];
            for (let i in rows) {
            ...
            if (replyIds.length > 0) {
                const cmts = await weiboDB.getCommentsByIds(replyIds);
                for (let i in cmts) {
                    const item = cmts[i];
                    userIds.push(item.user_id);
    ...
    public async getLikes(feedId:string):Promise<[boolean, Like[], string]>{
        let likes: Like[] = [];
        try {
            const rows = await weiboDB.getLikes(feedId);
            const userIds = [];
            for (let i in rows) {
                const item = rows[i];
                userIds.push(item.user_id);
    ...
    public async getBeenReposted(feedId:string):Promise<[boolean, BeenPosted[], string]>{
        ...
        try {
            const rows = await weiboDB.getBeenReposted(feedId);
            const userIds = [];
            for (let i in rows) {
                const item = rows[i];
                userIds.push(item.user_id);
    ...
    public async getTSR(type:string, id:string):Promise<[boolean, TSR, string]>{
        let tsr:TSR;
        try {
            const rows = await weiboDB.getTSR(type, id);
            if(rows.length <= 0){
                return [false, tsr, ''];
            }
            tsr = {
    ...
    public async saveComment(content:string, media:Media[], location:Location, replyToId:string, feedId:string, uid:string):Promise<[boolean, string]>{
        ...
        try {
            ...
            await weiboDB.saveComment(record);
            await weiboDB.IncrWeibo('comment_num', feedId);
            return [true, ''];
        }catch (e){
            return [false, userErrorMessage(e)];
        }
    ...
    private async  getUsersMap(uids:string[]):Promise<Map<string, User>>{
        ...
        let rows = await weiboDB.getUsers(userIds);
        for(let i in rows){
            const item = rows[i];
    ...
    private async getAttachmentsMap(type: string, thirdIds:string[]):Promise<Map<string, Media[]>>{
        ...
        let attachments = await weiboDB.getAttachments(type, thirdIds);
    ...
    private async getRetweetsMap(retweetIds: string[]):Promise<Map<string, Weibo>>{
        ...
        if( feedIds.length > 0 ){
            const rows = await weiboDB.getRetweets(feedIds);
            let attachmentIds = [];
            let userIds = [];
            for(let i in rows) {
                const item = rows[i];
        ...
        if( repostIds.length > 0 ){
            const retweets = await weiboDB.getRetweetsCompatible(repostIds);
            let attachmentIds = [];
            let userIds = [];
    ...
    private async getBiesMap():Promise<Map<string, By>>{
        ...
        const rows = await weiboDB.getBies();
        for(let i in rows){
