...
class NewsService{
    ...
    private async crawZaobao() {
        const response = await fetch('https://www.zaobao.com/realtime', {
            method: 'GET',
            headers: {
        ...
        let str = await NativeModules.RNHelper.parseNews('zaobao', 'list', body, '(//div[contains(@class, \'lg:w-1/2\')])[2]//a[./span]');
        ...
        str = await NativeModules.RNHelper.parseNews('zaobao', 'list', body, '(//div[contains(@class, \'lg:w-1/2\')])[1]//a[./span]');
        let items = JSON.parse(str);
        for (let i in items) {
            items[i].uid = 101;
            let domain = 'https://www.zaobao.com';
            if( !items[i].href.startsWith('/') ){
                domain += '/';
            }
            items[i].href = domain + items[i].href;
